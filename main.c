...
Node *findSmallerAnscestor(Node *root,Node* currentNode)
{
    Node*temp=root;
    Node*tempReturn;
...
Node* findPredescessor(Node * root, Node * currentNode)
{
    ...
    {
        return findSmallerAnscestor(root,currentNode);
    }
    return NULL;
...
Node * findBiggerAnscestor(Node *root,Node* currentNode)
{
    Node*temp=root;
    Node*tempReturn;
    while(temp!=currentNode)
...
Node * findSuccessor(Node * root, Node * currentNode)
{
    ...
    {
        return findBiggerAnscestor(root,currentNode);
    }
...
Node * searchInTree( Node *root, char word[],Node *WordBefore,Node * constantRoot )
{
    if( root == NULL )
    {
        ...
        printf("%s  ",findPredescessor(constantRoot,WordBefore)->data);
        return NULL;
